...
function TcpServer.__index(target, key)
	if rawget(TcpServer, key) ~= nil then return TcpServer[key] end
...
function TcpServer:StartListening()
	DEBUG("Listening on tcp://" .. self.hostName .. ":" .. self.port .. " (Backlog queue size: " .. self.backlogQueueSize .. ")")
...
function TcpServer:TCP_SERVER_STARTED() DEBUG("[TcpServer] TCP_SERVER_STARTED triggered") end

function TcpServer:TCP_SERVER_STOPPED() DEBUG("[TcpServer] TCP_SERVER_STOPPED triggered") end
