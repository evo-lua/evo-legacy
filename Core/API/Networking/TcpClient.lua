...
function TcpClient.__index(target, key)
	if rawget(TcpClient, key) ~= nil then return TcpClient[key] end
	if TcpSocket[key] ~= nil then return TcpSocket[key] end
...
function TcpClient:TCP_CONNECTION_ESTABLISHED() DEBUG("[TcpClient] TCP_CONNECTION_ESTABLISHED triggered") end
function TcpClient:TCP_SESSION_STARTED() DEBUG("[TcpClient] TCP_SESSION_STARTED triggered") end
function TcpClient:TCP_WRITE_SUCCEEDED(chunk) DEBUG("[TcpClient] TCP_WRITE_SUCCEEDED triggered", chunk) end
function TcpClient:TCP_WRITE_FAILED(errorMessage, chunk) DEBUG("[TcpClient] TCP_WRITE_FAILED triggered", errorMessage, chunk) end
